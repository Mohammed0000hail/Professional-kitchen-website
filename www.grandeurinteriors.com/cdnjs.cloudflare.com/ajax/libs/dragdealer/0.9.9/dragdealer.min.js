! function(t, i) {
    "function" == typeof define && define.amd ? define(i) : "object" == typeof module && module.exports ? module.exports.Dragdealer = i() : t.Dragdealer = i()
}(this, function() {
    function t(t) {
        ...
        for (var s = 0; s < i.length; s++)
            if (void 0 !== e[i[s] + t]) return i[s] + t
    }

    function i(t) {
        l.backfaceVisibility && l.perspective && (t.style[l.perspective] = "1000px", t.style[l.backfaceVisibility] = "hidden")
    }
    var e = function(t, i) {
        this.options = this.applyDefaults(i || {}), this.bindMethods(), this.wrapper = this.getWrapperElement(t), this.wrapper && (this.handle = this.getHandleElement(this.wrapper, this.options.handleClass), this.handle && (this.init(), this.bindEventListeners()))
    };
    e.prototype = {
        defaults: {
            disabled: !1,
        ...
        init: function() {
            this.options.css3 && i(this.handle), this.value = {
                prev: [-1, -1],
        ...
        calculateStepRatios: function() {
            ...
            if (this.options.steps >= 1)
                for (var i = 0; i <= this.options.steps - 1; i++) this.options.steps > 1 ? t[i] = i / (this.options.steps - 1) : t[i] = 0;
            return t
        },
        ...
        onDocumentMouseMove: function(t) {
            t.clientX - this.dragStartPosition.x == 0 && t.clientY - this.dragStartPosition.y == 0 || (r.refresh(t), this.dragging && (this.activity = !0, a(t)))
        },
        onWrapperTouchMove: function(t) {
            r.refresh(t), this.activity || !this.draggingOnDisabledAxis() ? (a(t), this.activity = !0) : this.dragging && this.stopDrag()
        },
        ...
        setStep: function(t, i, e) {
            this.setValue(this.options.steps && t > 1 ? (t - 1) / (this.options.steps - 1) : 0, this.options.steps && i > 1 ? (i - 1) / (this.options.steps - 1) : 0, e)
        },
        setValue: function(t, i, e) {
            this.setTargetValue([t, i || 0]), e && (this.groupCopy(this.value.current, this.value.target), this.updateOffsetFromValue(), this.callAnimationCallback())
        },
        startTap: function() {
            !this.disabled && this.options.tapping && (this.tapping = !0, this.setWrapperOffset(), this.setTargetValueByOffset([r.x - this.offset.wrapper[0] - this.handle.offsetWidth / 2, r.y - this.offset.wrapper[1] - this.handle.offsetHeight / 2]))
        },
        stopTap: function() {
        ...
        startDrag: function() {
            this.disabled || (this.dragging = !0, this.setWrapperOffset(), this.dragStartPosition = {
                x: r.x,
                y: r.y
            }, this.offset.mouse = [r.x - u.get(this.handle)[0], r.y - u.get(this.handle)[1]], this.wrapper.className.match(this.options.activeClass) || (this.wrapper.className += " " + this.options.activeClass), this.callDragStartCallback())
        },
        stopDrag: function() {
            if (!this.disabled && this.dragging) {
                this.dragging = !1;
                var t = [0 === this.bounds.availWidth ? 0 : (r.x - this.dragStartPosition.x) / this.bounds.availWidth, 0 === this.bounds.availHeight ? 0 : (r.y - this.dragStartPosition.y) / this.bounds.availHeight],
                    i = this.groupClone(this.value.current);
                if (this.options.slide) {
                    var e = this.change;
                    i[0] += 4 * e[0], i[1] += 4 * e[1]
                }
                this.setTargetValue(i), this.wrapper.className = this.wrapper.className.replace(" " + this.options.activeClass, ""), this.callDragStopCallback(t)
            }
        },
            ...
            if (!t || this.dragging) {
                if (this.dragging) {
                    var e = this.groupClone(this.value.target),
                        s = [r.x - this.offset.wrapper[0] - this.offset.mouse[0], r.y - this.offset.wrapper[1] - this.offset.mouse[1]];
                    this.setTargetValueByOffset(s, this.options.loose), this.change = [this.value.target[0] - e[0], this.value.target[1] - e[1]]
                }(this.dragging || i) && this.groupCopy(this.value.current, this.value.target), (this.dragging || this.glide() || i) && (this.updateOffsetFromValue(), this.callAnimationCallback())
            }
        },
        glide: function() {
        ...
        setTargetValue: function(t, i) {
            ...
            this.groupCopy(this.value.target, e), this.offset.target = this.getOffsetsByRatios(e), this.callTargetCallback()
        },
        ...
        getProperValue: function(t) {
            ...
            return i[0] = Math.max(i[0], 0), i[1] = Math.max(i[1], 0), i[0] = Math.min(i[0], 1), i[1] = Math.min(i[1], 1), (!this.dragging && !this.tapping || this.options.snap) && this.options.steps > 1 && (i = this.getClosestSteps(i)), i
        },
        ...
        getClosestStep: function(t) {
            for (var i = 0, e = 1, s = 0; s <= this.options.steps - 1; s++) Math.abs(this.stepRatios[s] - t) < e && (e = Math.abs(this.stepRatios[s] - t), i = s);
    ...
    for (var s = function(t, i) {
            ...
            set: function(t) {
                var i = this.x,
                    e = this.y;
                t.clientX || t.clientY ? (this.x = t.clientX, this.y = t.clientY) : (t.pageX || t.pageY) && (this.x = t.pageX - document.body.scrollLeft - document.documentElement.scrollLeft, this.y = t.pageY - document.body.scrollTop - document.documentElement.scrollTop), this.xDiff = Math.abs(this.x - i), this.yDiff = Math.abs(this.y - e)
            }
        }, u = {
            get: function(t) {
                var i = {
        ...
        }, p = ["webkit", "moz"], c = window.requestAnimationFrame, f = window.cancelAnimationFrame, d = 0; d < p.length && !c; ++d) c = window[p[d] + "RequestAnimationFrame"], f = window[p[d] + "CancelAnimationFrame"] || window[p[d] + "CancelRequestAnimationFrame"];
